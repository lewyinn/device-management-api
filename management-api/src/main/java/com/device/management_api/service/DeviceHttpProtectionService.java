...
@Service
public class DeviceHttpProtectionService {
    ...
    public ResponseEntity<?> registerRateLimit(HttpServletRequest request) {
    ...
    public ResponseEntity<?> readThrottling(HttpServletRequest request) {
        return checkThrottling(
                "Device read endpoint",
                "device_read:" + clientIp(request),
                1_000
    ...
    private ResponseEntity<?> checkRateLimit(
        ...
        int retryAfterSeconds = Math.max(1, (int) Math.ceil((state.resetAt() - now) / 1000.0));
    ...
    private ResponseEntity<?> checkThrottling(
        ...
        if (lastRequestAt != null && now - lastRequestAt < throttleMs) {
            ...
            HttpHeaders headers = new HttpHeaders();
            headers.add("Retry-After", String.valueOf(Math.max(1, (int) Math.ceil(retryAfterMs / 1000.0))));
            ...
            return ResponseEntity
                    ...
                    .body(new ProtectionErrorResponse(
                            "Request throttled",
                            "throttling",
                            endpointName + " only accepts one request every " + throttleMs + " milliseconds",
                            null,
                            retryAfterMs
                    ));
        }
