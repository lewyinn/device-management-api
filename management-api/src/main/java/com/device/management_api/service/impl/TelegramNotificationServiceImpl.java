...
public class TelegramNotificationServiceImpl implements TelegramNotificationService {
    ...
    @Override
    @Async
    public void sendDeviceRegisteredNotification(Device device) {
        ...
        try {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                logger.error(
                        "Telegram device registration notification failed with status {}: {}",
                        response.statusCode(),
                        response.body()
                );
            }
    ...
    private String formatDeviceRegisteredMessage(Device device) {
        return String.join(
                "\n",
